...
export const s3Client = new S3Client({
  endpoint: process.env.MINIO_ENDPOINT ? `http://${process.env.MINIO_ENDPOINT}` : process.env.endpoint,
  region: process.env.region || 'us-east-1',
  credentials: {
    accessKeyId: process.env.MINIO_ACCESS_KEY || process.env.accessKeyId,
    secretAccessKey: process.env.MINIO_SECRET_KEY || process.env.secretAccessKey
  },
  forcePathStyle: process.env.forcePathStyle === 'true' || true  // MinIO requires this
});
...
export async function downloadFile(objectKey, localTempPath) {
  ...
  const fileStream = fs.createWriteStream(localTempPath);
